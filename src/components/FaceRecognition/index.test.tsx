...
describe('FaceRecognition', () => {
  ...
  test('Image displays with bounding boxes', async () => {
    ...
    // screen.debug()
    const boundingBoxes = await waitFor(() =>
      screen.findAllByTestId('bounding-box')
    )
    ...
    boundingBoxes.forEach((box, index) => {
      ...
      //add mouseOver event for the box for sentimentLabel to show up
      const sentimentLabel = within(box).getByDisplayValue(
